...
void functions(void)
{
	for(uint8_t x = 0; x < COLUMNS; x++)
	{
		...
		if(keypress.pressed)
		{
			...
			#ifdef ENABLE_NEXTTAB
			if(keypress.row == nexttab_key.row
			&& keypress.column == nexttab_key.column)
			{LCTRL(KEY_TAB);}
			#endif

			#ifdef ENABLE_PREVTAB
			if(keypress.row == prevtab_key.row
			&& keypress.column == prevtab_key.column)
			{LCTRL(LSHIFT(KEY_TAB));}
			#endif
